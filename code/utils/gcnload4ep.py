...
def preprocess_adj(adj, is_decentRL=False):
    """Preprocessing of adjacency matrix for simple GCN gnn and conversion to tuple representation."""
    if is_decentRL:
        adj_normalized = normalize_adj(adj + 1e-10*sp.eye(adj.shape[0]))
    else:
...
def generate_2hop_triples(kg, linked_ents=None):
    ...
    existed_patterns = np.unique(np.array(triples)[:, [0, -1]], axis=-1)
    two_hop_triple_df = pd.merge(triple_df, triple_df, left_on='t', right_on='h')
    
    candidates = two_hop_triple_df.values[:, [0, -1]]
    mask = ~in2d(candidates, existed_patterns)

    selected = two_hop_triple_df[mask]
    
    selected_rel_patterns = np.stack(selected.groupby(['r_x','r_y']).size().index.values).astype(candidates.dtype)
    selected_rel_patterns = selected_rel_patterns[int(.8*len(selected_rel_patterns)):]
    rel_mask = in2d(selected[['r_x','r_y']].values, selected_rel_patterns)

    final_selected = selected[rel_mask].values

    print(final_selected.shape)
    final_selected[:, 1] += final_selected[:, -2]

    final_twohop_triples = np.unique(final_selected[:, [0, 1, -1]], axis=-1)
    print(final_twohop_triples.shape)
    return list(final_twohop_triples)

    # print(selected.shape, two_hop_triple_df.columns)
...
def gcn_load_data(triples, total_e_num, total_r_num, is_decentRL=False, return_radj=False, is_two=False):
    class KG:
        def __init__(self, triples):
            self.triples = self.triple_list = triples
    ...
    def generate_mats():
        ...
        def generate_radj(triple_list):
            df = pd.DataFrame(triple_list, columns=['h_id','r_id','t_id'])
#             print(df.shape,df.max(axis=0))
            er_adj = df.groupby(['h_id', 'r_id']).size().reset_index()
            re_adj = df.groupby(['r_id', 't_id']).size().reset_index()

            # total_e_num, total_r_num = ent_num, rel_num

            er_adj = coo_matrix((np.ones_like(er_adj.values[:, -1]), (er_adj.values[:, 0], er_adj.values[:, 1])), shape=(total_e_num, total_r_num))
            re_adj = coo_matrix((np.ones_like(re_adj.values[:, -1]), (re_adj.values[:, 0], re_adj.values[:, 1])), shape=(total_r_num, total_e_num))
            def normalize_adj(adj):
                rowsum = 1 / (adj.sum(1) + 1e-15) 
                return adj.multiply(rowsum)
            return (sparse_to_tuple(normalize_adj(er_adj)), sparse_to_tuple(normalize_adj(re_adj)))

        radj = [generate_radj(triples)]

        one_adj, _ = no_weighted_adj(total_e_num, triples, is_two_adj=False, is_decentRL=is_decentRL)
        adj = [one_adj]

        if is_two:
            # for relation matrix, we only use one-hop neighbors
            radj.append(generate_radj(triples))

            two_hop_triples = generate_2hop_triples(kg)
            # triples = remove_unlinked_triples(triples, linked_ents)
            two_adj, _ = no_weighted_adj(total_e_num, two_hop_triples, is_two_adj=False, is_decentRL=is_decentRL)
            adj.append(two_adj)
            # total_r_num = len(np.unique(np.array(triples)[:, 1]))
        else:
            radj += radj
            adj += adj
        return adj, radj
            
    adj, radj = generate_mats()
    
    results = [adj, ]
    #or return_radj
    if is_decentRL or return_radj:
        results.append(radj)
...
def no_weighted_adj(total_ent_num, triple_list, is_two_adj=False, is_decentRL=False):
    start = time.time()
    edge = dict()
    ...
    one_adj = preprocess_adj(one_adj, is_decentRL=is_decentRL)
    ...
    two_adj = preprocess_adj(two_adj, is_decentRL=is_decentRL)
